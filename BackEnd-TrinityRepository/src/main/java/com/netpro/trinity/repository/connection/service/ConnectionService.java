...
import com.netpro.notes.driver.NotesDriver;
...
public class ConnectionService {
	...
	public String testNotesConnection(NotesConnection notesConn) throws Exception{
		String result = "Test Notes Connection fail";
		
		String serverName = notesConn.getNotesServerName();
		if(null == serverName || serverName.trim().isEmpty())
			serverName = "";
		
		String hostIP = notesConn.getNotesHostIP();
		if(null == hostIP || hostIP.trim().isEmpty())
			throw new Exception("Host IP can not be empty!");
		
		String dbName = notesConn.getNotesDBName();
		if(null == dbName || dbName.trim().isEmpty())
			throw new Exception("DB Name can not be empty!");
		
		String notesIor = notesConn.getNotesIor();
		if(null == notesIor || notesIor.trim().isEmpty())
			notesIor = "";
		
		String userID = notesConn.getUserid();
		if(null == userID || userID.trim().isEmpty())
			throw new Exception("User ID can not be empty!");
		
		String pwd = notesConn.getPassword();
		if(null == pwd || pwd.trim().isEmpty())
			throw new Exception("Password can not be empty!");
		
		NotesDriver.connect(hostIP, notesIor, userID, pwd, serverName, dbName);
		
		result = "Test Notes Connection Success";
		return result;
	}
