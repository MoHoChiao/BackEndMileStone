package com.netpro.trinity.service.util.status;

public class TrinityServiceStatusMsg {
	public static String LOGIN_ERROR = "Login Error!";
	public static String LOGIN_SUCCESS = "Login Success.";
	public static String LOGOUT_ERROR = "Logout Fail.";
	public static String LOGOUT_SUCCESS = "Logout Success.";
	public static String VALIDATE_ERROR = "Validate Fail.";
	public static String VALIDATE_SUCCESS = "Validate Success.";
	public static String ACCOUNT_EMPTY = "User Account can not be empty!";
	public static String PSW_EMPTY = "User PSW can not be empty!";
	public static String PSW_NEAR_EXPIRE = "Password is near expiration date!";
	public static String ACCOUNT_CHECK = "Please Check Your Account!";
	public static String RESET_PSW = "Please Reset your password!";
	public static String RESET_PSW_SUCCESS = "Password changed, please login with new password.";
	public static String RESET_PSW_ERROR = "Change password failed!";
	public static String Session_Expired = "Session expired, please login to get reset token again!";
}
