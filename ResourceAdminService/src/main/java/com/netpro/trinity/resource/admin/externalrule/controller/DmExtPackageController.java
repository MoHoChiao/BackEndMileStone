package com.netpro.trinity.resource.admin.externalrule.controller;
...
import com.netpro.trinity.resource.admin.externalrule.service.DmExtPackageService;
...
public class DmExtPackageController {
	...
	public ResponseEntity<?> findAllPackages(HttpServletRequest request, Boolean withoutDetail) {
		try {
			...
			if (this.authzService.checkFuncPermission(peopleId, "extrule", "view")) {
	...
	public ResponseEntity<?> addPackage(HttpServletRequest request, @RequestBody Dmextpackage p) {
		try {
			String peopleId = ACUtil.getUserIdFromAC(request);
			if (this.authzService.checkFuncPermission(peopleId, "extrule", "edit")) {
